...
public class X5Pool {
    ...
    public String getNextActivity(int sId) {
        String i = sId == 0 ? "Temp" : String.valueOf(refreshOrder(sId));
        Log.i(X5Pool.TAG, "get pool no: " + i + "  with sId:" + sId);
        return "com.demo.utils.rnx5.x5.X5Activity" + i;
    }
    ...
    public int refreshOrder(int sId) {
        ...
        if ((index = this.sIdPool.indexOf(sId)) < 0) {
            ...
            Log.i(X5Pool.TAG, "new one: " + index + "  with sId:" + sId);
        } else {
            orderPool.add(orderPool.remove(orderPool.indexOf(index)));
            Log.i(X5Pool.TAG, "refreshOrder: " + index + "  with sId:" + sId);
        }
        return index;
    }
